...
-- Função para o Auto Demon Fruit
local AutoDemonFruitEnabled = false

Tab2:AddToggle({
    Name = "Auto Demon Fruit",
    Default = false,
    Callback = function(Value)
        AutoDemonFruitEnabled = Value
    end
})

spawn(function()
    while true do
        if AutoDemonFruitEnabled then
            if DevilFruit then
...
local OldWorldAutoFarming = false
